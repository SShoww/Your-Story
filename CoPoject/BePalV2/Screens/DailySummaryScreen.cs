...
public sealed class DailySummaryScreen : IScreen
{
    ...
    public void Draw(GameTime gameTime, SpriteBatch batch)
    {
        ...
        Rectangle gradeStamp = new(doc.Right - 360, doc.Y + 115, 300, 64);
        ...
        batch.DrawString(_ctx.Font, gradeText, new Vector2(gradeStamp.Center.X - (gSize.X * 1.5f) / 2f, gradeStamp.Center.Y - (gSize.Y * 1.5f) / 2f), gradeColor, 0f, Vector2.Zero, 1.5f, SpriteEffects.None, 0f);
        ...
        batch.DrawString(_ctx.Font, $"- Daily Shelter Subsidy:          +{EconomyManager.DailySubsidyAmount} G (On Wake)", new Vector2(doc.X + 70, fy + 38), new Color(40, 110, 40));
        batch.DrawString(_ctx.Font, $"- Care Performance Bonus:         +{run.LastGoldReward} G", new Vector2(doc.X + 70, fy + 72), new Color(40, 110, 40));
        batch.DrawString(_ctx.Font, $"- Research Points Earned Today:   +25 PTS", new Vector2(doc.X + 70, fy + 106), new Color(30, 80, 140));
        batch.DrawString(_ctx.Font, $"- Current Wallet Balance:         {run.Economy.Gold} G", new Vector2(doc.X + 70, fy + 140), new Color(20, 20, 20));
